...
rankGenes <- function(refVrefFiles,testVrefFiles,inFolder,outFolder,
                      ...
                      chrsOfInterest=paste("chr",1:19,sep=""),minGUs=10){
...
rankGenePromsJSD <- function(testVrefFiles,inFolder,outFolder,tName="test",rName="ref",
  ...
  for (ind in 1:numAltComp){
    ...
    sortedOutput <- AnnotationDbi::select(Mus.musculus, key=as.character(proms[sortedIndices]$gene_id), keytype="ENTREZID", columns=c("SYMBOL"))
...
rankGeneBodiesAndProms <- function(refVrefFiles,testVrefFiles,inFolder,outFolder,
                                   tName,rName,chrsOfInterest = paste("chr",1:19,
  ...
  # find names of genes
  nameMapping <- AnnotationDbi::select(Mus.musculus,key=as.character(mergedDF$gene_id),
                        ...
                        chrsOfInterest=paste("chr",1:19,sep=""))
{
  ...
  genome.seqinfo <- Seqinfo(genome="mm10")
  ...
  symbol.table <- select(Mus.musculus,key=annots.genes.gr$gene_id,keytype="ENTREZID",columns=c("SYMBOL"))
